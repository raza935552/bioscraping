...
const NO = [
  ...
  /\bleave me alone\b/, /\bspam\b/, /\bno\b[.!]*$/, /^no\b/, /\bi'?ll pass\b/, /\bdecline\b/, /\bexclusive\b/, /\bcan'?t (work|partner)\b/,
];
const MORE = [
...
export function classifyReplyKeywords(raw: string): ReplySuggestion {
  ...
  const yes = /\bnot interested\b/.test(t) ? Math.max(0, yesRaw - 1) : yesRaw;
